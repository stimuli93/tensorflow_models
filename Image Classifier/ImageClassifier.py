...
class ConvolutionalClassifier(object):
    def __init__(self, img_dims, n_classes=2, ckpt_dir="./ckpt_dir", summary_dir="/tmp/convClf_logs"):
        """
        :param img_dims: a tuple of (width, height, channel) of input image.
    ...
    def _initialize_weights(self, img_dims, n_classes):
        ...
        all_weights['W_conv1'] = tf.Variable(tf.truncated_normal(shape=[3, 3, img_dims[2], 4], stddev=0.1),
                                             name="W_conv1")
        all_weights['b_conv1'] = tf.Variable(tf.constant(0.1, shape=[4]), name="b_conv1")
        all_weights['W_conv2'] = tf.Variable(tf.truncated_normal(shape=[3, 3, 4, 8], stddev=0.1), name="W_conv2")
        all_weights['b_conv2'] = tf.Variable(tf.constant(0.1, shape=[8]), name="b_conv2")
        all_weights['W_conv3'] = tf.Variable(tf.truncated_normal(shape=[3, 3, 8, 16], stddev=0.1), name="W_conv3")
        all_weights['b_conv3'] = tf.Variable(tf.constant(0.1, shape=[16]), name="b_conv3")
        all_weights['W_fc1'] = tf.Variable(tf.truncated_normal(shape=[(img_dims[0]//8)*(img_dims[1]//8)*16, 256],
                                                               stddev=0.1), name="W_fc1")
        all_weights['b_fc1'] = tf.Variable(tf.constant(0.1, shape=[256]), name="b_fc1")
        all_weights['W_fc2'] = tf.Variable(tf.truncated_normal(shape=[256, 64], stddev=0.1), name="W_fc2")
        all_weights['b_fc2'] = tf.Variable(tf.constant(0.1, shape=[64]), name="b_fc2")
        all_weights['W_fc3'] = tf.Variable(tf.truncated_normal(shape=[64, n_classes], stddev=0.1), name="W_fc3")
        all_weights['b_fc3'] = tf.Variable(tf.constant(0.1, shape=[n_classes]), name="b_fc3")
